...
def call(Map config){
    ...
    withCredentials([string(credentialsId: "${secretToken}", variable: "setup")]) {
         ...
         sh "set +x; docker run --rm ${AWS_CREDENTIALS} -e AWS_PROFILE=${awsProfile} -e AWS_REGION=${awsRegion} -v ${workspace}/:/code -w /code/${workDir} ${terraformDockerImage} sh -c 'terraform ${action} ${extraParameter} && rm -rf .terraform'"   
